...
- `app/layout.tsx` — loads the three type roles via `next/font/google`: **Newsreader** (display — `font-display`, via the `--font-newsreader` variable; replaced Fraunces because its WONK axis was hard to read at large sizes), **Space Grotesk** (`font-grotesk`, used for labels/nav/metrics), **Inter** (body). Each is wired to the CSS variable the tokens reference. Note: the **CV PDF still uses Fraunces** (`workAssets/cv-source/cv.html`) — regenerate it with Newsreader if you want the download to match the site.
